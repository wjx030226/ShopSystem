...
public class Main {
    public static void main(String[] args){
        ...
        //读取csv文件
        products.readFile("file\\GoodsMessage.csv", goods);
        user.readFile("file\\UsersMessage.csv", users);
        shopCar.readFile("file\\CarsMessage.csv", shopHistory);
        while(true){
            System.out.println("============================");
            ...
            switch (x) {
                        ...
                        switch (y) {
                            ...
                            case 3 -> {
                                do{
                                    ...
                                    switch (z) {
                                        case 1 -> {
                                            if(userID != 0)  user.change(users, userID);
                                            else System.out.println("您还未登录账号，请先登录！");
                                        }
                ...
                case 3 -> {
                    //将信息写入csv文件
                    products.writeFile("file\\GoodsMessage.csv", goods);
                    user.writeFile("file\\UsersMessage.csv", users);
                    shopCar.writeFile("file\\CarsMessage.csv", shopHistory);
                    System.exit(1);
                }
                default -> System.out.println("输入的选择无效，请重新输入！");
            }
